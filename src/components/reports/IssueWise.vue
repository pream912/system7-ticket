<template>
    <v-container>
        ...
        <v-row>
            <v-col cols="4">
                <v-autocomplete filled label="Issue" item-value="name" v-model="issue" item-text="name" :items="issues"></v-autocomplete>
            </v-col>
            <v-col cols="4">
    ...
    computed: {
        ...
        filt_tickets() {
            if(this.issue == null) {
                ...
                let st = this.tickets.filter((item) => {
                    return item.issue == this.issue
                })
                return this.prepTickets(st)
            }
        },
